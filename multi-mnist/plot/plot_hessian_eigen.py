...
def crunch_hessian_eigs(surf_file, net, w, s, d, dataloader, comm, rank, args):
    ...
    inds, coords, inds_nums = scheduler.get_job_indices(
        max_eig, xcoordinates, ycoordinates, comm
    )
    ...
    for count, ind in enumerate(inds):
        ...
        maxeig, mineig, iter_count = hess_vec_prod.min_max_hessian_eigs(
            net, dataloader, criterion, rank=rank, use_cuda=args.cuda, verbose=True
        )
        ...
        print(
            "rank: %d %d/%d  (%0.2f%%)  %d\t  %s \tmaxeig:%8.5f \tmineig:%8.5f \titer: %d \ttime:%.2f \tsync:%.2f"
            % (
                rank,
                count + 1,
                len(inds),
                100.0 * (count + 1) / len(inds),
                ind,
                str(coord),
                maxeig,
                mineig,
...
if __name__ == "__main__":
    ...
    if args.plot and rank == 0:
        if args.y:
            ...
            plot_1D.plot_1d_eig_ratio(
                surf_file, args.xmin, args.xmax, "min_eig", "max_eig"
            )
