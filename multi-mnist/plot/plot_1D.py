...
def plot_1d_loss_err(surf_file, xmin=-1.0, xmax=1.0, loss_max=5, log=False, show=False):
    ...
    ax2 = ax1.twinx()
    if log:
        (tr_loss,) = ax1.semilogy(
            x, train_loss, "b-", label="Training loss", linewidth=1
        )
    else:
    ...
    if "test_loss" in f.keys():
        ...
        test_acc = f["test_acc"][:]
        if log:
            (te_loss,) = ax1.semilogy(
                x, test_loss, "b--", label="Test loss", linewidth=1
            )
        else:
    ...
    ax2.set_ylim(0, 100)
    pp.savefig(
        surf_file + "_1d_loss_acc" + ("_log" if log else "") + ".pdf",
        dpi=300,
        bbox_inches="tight",
        format="pdf",
    )
    ...
    pp.ylim(0, loss_max)
    pp.savefig(
        surf_file + "_1d_train_loss" + ("_log" if log else "") + ".pdf",
        dpi=300,
        bbox_inches="tight",
        format="pdf",
    )
    ...
    pp.savefig(
        surf_file + "_1d_train_err.pdf", dpi=300, bbox_inches="tight", format="pdf"
    )
...
def plot_1d_loss_err_repeat(
    prefix, idx_min=1, idx_max=10, xmin=-1.0, xmax=1.0, loss_max=5, show=False
):
    """
    ...
    ax2.set_ylim(0, 100)
    pp.savefig(
        prefix + "_1d_loss_err_repeat.pdf", dpi=300, bbox_inches="tight", format="pdf"
    )
...
def plot_1d_eig_ratio(
    surf_file, xmin=-1.0, xmax=1.0, val_1="min_eig", val_2="max_eig", ymax=1, show=False
):
    print("------------------------------------------------------------------")
    ...
    pp.ylim(0, ymax)
    pp.savefig(
        surf_file + "_1d_eig_abs_ratio.pdf", dpi=300, bbox_inches="tight", format="pdf"
    )
    ...
    pp.ylim(0, ymax)
    pp.savefig(
        surf_file + "_1d_eig_ratio.pdf", dpi=300, bbox_inches="tight", format="pdf"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--surf_file", "-f", default="", help="The h5 file contains loss values"
    )
    parser.add_argument(
        "--log", action="store_true", default=False, help="logarithm plot"
    )
    ...
    parser.add_argument(
        "--prefix", default="", help="The common prefix for surface files"
    )
    parser.add_argument(
        "--idx_min", default=1, type=int, help="min index for the surface file"
    )
    parser.add_argument(
        "--idx_max", default=10, type=int, help="max index for the surface file"
    )

    args = parser.parse_args()

    if args.prefix:
        plot_1d_loss_err_repeat(
            args.prefix,
            args.idx_min,
            args.idx_max,
            args.xmin,
        ...
        plot_1d_loss_err(
            args.surf_file, args.xmin, args.xmax, args.loss_max, args.log, args.show
        )
