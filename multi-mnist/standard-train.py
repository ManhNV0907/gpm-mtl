...
# shared_scheduler = torch.optim.lr_scheduler.MultiStepLR(
#     shared_optimizer, milestones=[15, 30, 45, 60, 75, 90, 105, 120, 135, 150, 165, 180, 195], gamma=0.5
# )

# classifier_scheduler = torch.optim.lr_scheduler.MultiStepLR(
#     classifier_optimizer, milestones=[15, 30, 45, 60, 75, 90, 105, 120, 135, 150, 165, 180, 195], gamma=0.5
# )
...
def train(epoch):
    all_losses_1 = 0
    all_losses_2 = 0
    # write_log("*" * 15 + f" Epoch {epoch} " + "*" * 15, log_file)
    for (it, batch) in tqdm(
        ...
        for name, param in model.named_parameters():
            if "task" in name:
                ...
                task2_shared_grad.append(param.grad.detach().data.clone().flatten())

            param.grad.zero_()
        task2_shared_grad = torch.cat(task2_shared_grad, dim=0)

        # task1_shared_grad = torch.nn.functional.normalize(task1_shared_grad, dim=0)
        # task2_shared_grad = torch.nn.functional.normalize(task2_shared_grad, dim=0)

        shared_grads = [task1_shared_grad, task2_shared_grad]

        with torch.no_grad():
            sol, _ = MinNormSolver.find_min_norm_element(shared_grads)

        shared_grad = sol[0] * shared_grads[0] + sol[1] * shared_grads[1]
        index_w = 0
        index_shared_grad = 0
